...
export default function Communities() {
  ...
  function CommunityCard({ c, joined, featured = false }) {
    ...
    return (
      <div className={`group rounded-2xl border p-6 transition-all hover:shadow-lg hover:scale-[1.02] ${featured ? 'border-blue-200 bg-blue-50' : 'hover:bg-[var(--hover)]'}`} 
           style={{ borderColor: featured ? '#dbeafe' : 'var(--border)', background: featured ? '#eff6ff' : 'var(--card-bg)' }}>
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                <Link to={`/communities/${c.slug || c.id}`} 
                      className="text-lg font-semibold text-[var(--text)] hover:text-blue-600 transition-colors group-hover:underline">
                  {c.name}
                </Link>
  ...
  function FeedItem({ p }) {
    ...
    return (
      <article className="bg-[var(--card-bg)] rounded-2xl border p-6 hover:bg-[var(--hover)] transition-colors" 
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <Link to={`/communities/${communityNames[p.community_id] || p.community_id}`} 
              ...
              in {communityNames[p.community_id] || 'Community'}
            </Link>
          </div>
        </div>
        ...
        {p.kind === 'text' && p.body && (
          <p className="text-[var(--text)] mb-4 leading-relaxed">{p.body}</p>
        )}
        ...
        {p.kind === 'photo' && p.photo_id && (
          <div className="mb-4">
            ...
            {p.body && (
              <p className="text-[var(--text)] mt-3 leading-relaxed">{p.body}</p>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-6 pt-3 border-t border-[var(--border)]">
          <button 
            ...
            className={`flex items-center gap-2 text-sm transition-colors ${
              likedByMe.has(p.id) ? 'text-red-500' : 'text-[var(--muted)] hover:text-red-500'
            }`}
          >
          ...
          <form onSubmit={createCommunity} className="p-6 space-y-4">
            ...
            <div>
              ...
              <textarea 
                ...
                className="mt-1 w-full rounded-xl bg-[var(--hover)] px-3 py-2 text-sm outline-none border border-[var(--border)] focus:border-blue-500 transition-colors" 
